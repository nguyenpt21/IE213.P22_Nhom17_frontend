...
const BookingsModal = ({ visible, onCancel, productId, productType, productName }) => {
    ...
    const fetchBookings = async () => {
        ...
        try {
            let response;
            if (productType === 'tour') {
                response = await dashboardService.getBookingsByTour(productId, pagination.limit, pagination.page);
            } else if (productType === 'hotel') {
                response = await dashboardService.getBookingsByHotel(productId, pagination.limit, pagination.page);
            }
    ...
    if (loading) {
        return (
            <Modal
                ...
                width={900}
                footer={null}
                title={`Danh sách booking - ${productName}`}
            >
                ...
                {bookings.length > 0 ? (
                                    ...
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex-1">
                                            <h4 className="text-lg font-semibold text-gray-900">
                                                {booking.name || `${booking.userId?.firstName || ''} ${booking.userId?.lastName || ''}`}
                                            </h4>
                                            <p className="text-sm text-gray-600">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-sm text-gray-700">
                                                {booking.email || booking.userId?.email || 'Chưa có email'}
                                            </span>
                                        </div>
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-gray-600">
                            {productType === 'tour' ? 'Tour này chưa có booking nào' : 'Khách sạn này chưa có booking nào'}
                        </p>
                    </div>
                )}
            </div>
